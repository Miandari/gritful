...
import { Infinity, Clock, CalendarClock } from 'lucide-react';
...
import { parseLocalDate, getLocalDateFromISO, calculateDisplayStreak, getDaysUntilStart } from '@/lib/utils/dates';
...
export function ActiveChallengeCard({
  ...
  const daysUntilStart = isUpcoming ? getDaysUntilStart(challenge.starts_at) : 0;
  ...
  return (
    <Card
      className={cn(
        ...
        isUpcoming && "border-2 border-blue-400 bg-blue-50/30 dark:bg-blue-950/20"
      )}
    >
      ...
      <CardHeader className={isCreator ? 'pt-8' : ''}>
        <div className="flex items-start justify-between">
          ...
          {isUpcoming && (
            <Badge
              variant="outline"
              className="ml-2 shrink-0 flex items-center gap-1 bg-blue-50 text-blue-700 border-blue-300 dark:bg-blue-950 dark:text-blue-400 dark:border-blue-600"
            >
              <CalendarClock className="h-3 w-3" />
              {daysUntilStart === 1 ? 'Starts tomorrow' : `Starts in ${daysUntilStart}d`}
            </Badge>
          )}
          {isGracePeriod && (
            <Badge
          ...
          {!isCreator && isOngoing && !isGracePeriod && !isUpcoming && (
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div>
                ...
                <div className="font-semibold">
                  {isOngoing ? 'Ongoing' : `${challenge.duration_days} days`}
                </div>
              </div>
              <div>
                <div className="text-muted-foreground text-xs">Status</div>
                <div className="font-semibold text-blue-600">Joined</div>
              </div>
            </div>
            <div className="pt-2">
